...
router.post("/", handlePolicies(["ADMIN", "PREMIUM"]), async (req, res) => {
  try {
...
router.put("/:id", handlePolicies(["ADMIN", "PREMIUM"]), verifyMDBID(["id"]), async (req, res) => {
  try {
...
router.delete("/:id", handlePolicies(["ADMIN", "PREMIUM"]), verifyMDBID(["id"]), async (req, res) => {
  try {
    const filter = { _id: req.params.id };
